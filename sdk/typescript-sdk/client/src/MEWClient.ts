...
export class MEWClient {
  ...
  private handleEnvelope(envelope: Envelope): void {
    ...
    if (envelope.kind === 'system/welcome') {
      ...
      payload.participants.forEach((p: Participant) => this.participants.set(p.id, p));
    ...
    if (envelope.kind === 'mew/proposal/reject') {
      const payload = envelope.payload as MewProposalRejectPayload;
      const pending = this.pendingProposals.get(payload.correlation_id);
      if (pending) {
    ...
    if (envelope.kind === 'mew/capability/grant') {
      const payload = envelope.payload as MewCapabilityGrantPayload;
    ...
    if (envelope.kind === 'mew/capability/revoke') {
      const payload = envelope.payload as MewCapabilityRevokePayload;
      if (payload.to === this.participantId) {
        this.capabilities = this.capabilities.filter((c) => !payload.capabilities.includes(c.id));
      }
  ...
  async sendChat(text: string, format: 'plain' | 'markdown' = 'plain'): Promise<void> {
    await this.send({ kind: 'chat', payload: { text, format } });
  }
  async request<T = any>(method: string, params?: any, target?: string): Promise<T> {
    ...
    const envelope: PartialEnvelope = {
      ...
      payload: { jsonrpc: '2.0', id: correlationId, method, params } as JsonRpcRequest,
    };
  ...
  async notify(method: string, params?: any, target?: string): Promise<void> {
    await this.send({
      ...
      to: target ? [target] : undefined,
      payload: { jsonrpc: '2.0', method, params } as JsonRpcNotification,
    });
  }
  async propose(capability: string, envelope: PartialEnvelope): Promise<Envelope> {
    const correlationId = uuidv4();
    const proposal: Proposal = { correlation_id: correlationId, capability, envelope };
    await this.send({ kind: 'mew/proposal', payload: { proposal } });
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
  ...
  async acceptProposal(correlationId: string): Promise<void> {
    await this.send({
      kind: 'mew/proposal/accept',
      payload: { correlation_id: correlationId, accepted_by: this.participantId },
    });
  }
  async rejectProposal(correlationId: string, reason?: string): Promise<void> {
    await this.send({
      kind: 'mew/proposal/reject',
      payload: { correlation_id: correlationId, rejected_by: this.participantId, reason },
    });
  }
  async grantCapabilities(to: string, capabilities: Capability[]): Promise<void> {
    await this.send({
      kind: 'mew/capability/grant',
      payload: { grant: { from: this.participantId, to, capabilities } },
    });
  }
  async revokeCapabilities(to: string, capabilityIds: string[]): Promise<void> {
    await this.send({
      kind: 'mew/capability/revoke',
      payload: { from: this.participantId, to, capabilities: capabilityIds },
    });
  }
  async pushContext(topic: string): Promise<void> {
    ...
    await this.send({
      kind: 'mew/context',
      context: { operation: 'push', topic, correlation_id: correlationId },
      payload: {},
    });
  }
  async popContext(): Promise<void> {
    ...
    await this.send({
      kind: 'mew/context',
      context: { operation: 'pop', correlation_id: correlationId },
      payload: {},
    });
  }
  async resumeContext(correlationId: string, topic?: string): Promise<void> {
    await this.send({
      kind: 'mew/context',
      context: { operation: 'resume', correlation_id: correlationId, topic },
      payload: {},
    });
  }
  ...
  private emit<K extends keyof ClientEventParams>(event: K, ...args: ClientEventParams[K]): void {
    const handlers = this.handlers[event] as Set<any>;
  ...
  private startHeartbeat(): void {
    ...
    this.heartbeatTimer = setInterval(async () => {
      try {
        await this.send({ kind: 'system/heartbeat', payload: {} });
      } catch {}
