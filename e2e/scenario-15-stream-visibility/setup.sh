#!/usr/bin/env bash
# Scenario 15 setup - prepares workspace for stream visibility on join tests
...
TEMPLATE_NAME=${TEMPLATE_NAME:-"scenario-15-stream-visibility"}
SPACE_NAME=${SPACE_NAME:-"scenario-15-stream-visibility"}
...
printf "%b\n" "${YELLOW}=== Scenario 15 Setup: Stream Visibility on Join ===${NC}"
...
mew init "${TEMPLATE_NAME}" --force --name "${SPACE_NAME}" --description "Scenario 15 - Stream Visibility" > init.log 2>&1
...
TEST_PORT=${TEST_PORT}
CLIENT_A_LOG=${WORKSPACE_DIR}/logs/client-a.log
CLIENT_B_LOG=${WORKSPACE_DIR}/logs/client-b.log
ENV
