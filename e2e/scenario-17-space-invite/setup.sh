#!/usr/bin/env bash
# Scenario 17 setup - prepares workspace for space/invite testing
...
TEMPLATE_NAME=${TEMPLATE_NAME:-"scenario-17-space-invite"}
...
mew init "${TEMPLATE_NAME}" --force --name "${SPACE_NAME}" --description "Scenario 17 - Space Invite" > init.log 2>&1

mkdir -p logs
: > logs/admin-output.log
: > logs/observer-output.log
...
TEST_PORT=${TEST_PORT}
ADMIN_LOG=${WORKSPACE_DIR}/logs/admin-output.log
OBSERVER_LOG=${WORKSPACE_DIR}/logs/observer-output.log
ENV
