...
gateway
  ...
  .action(async (options) => {
    ...
    app.post('/participants/:participantId/messages', async (req, res) => {
      ...
      const expectedToken =
        participantTokens.get(participantId) ||
    ...
    function registerHttpParticipants() {
      ...
      for (const [participantId, config] of Object.entries(spaceConfig.participants)) {
        ...
        const capabilityList = ensureBaselineCapabilities(config.capabilities || []);

        if (config.tokens && config.tokens.length > 0) {
          const token = config.tokens[0]; // Use first token
          participantTokens.set(participantId, token);
          participantCapabilities.set(participantId, capabilityList);
        ...
        const storedToken = tokenMap.get(participantId);
        if (storedToken) {
          participantTokens.set(participantId, storedToken);
          participantCapabilities.set(participantId, capabilityList);

          if (config.io === 'http' || !config.type || config.type === 'local') {
            console.log(`Registered local participant with stored token: ${participantId}`);
          }
        }
      }
    }
