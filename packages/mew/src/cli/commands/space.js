    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (
          ...
          pkg.workspaces.some((entry) => String(entry).includes('packages/mew'))
        ) {
          return dir;
        }
      } catch (error) {
...
function ensureLocalWorkspaceDependencies(spaceDir) {
  ...
  const targets = {
    '@mew-protocol/mew': path.join(repoRoot, 'packages/mew')
  };
  ...
  for (const [dep, targetPath] of Object.entries(targets)) {
    ...
    const fileSpec = normalized.startsWith('.') ? `file:${normalized}` : `file:./${normalized}`;

    if (packageJson.dependencies[dep] !== fileSpec) {
      packageJson.dependencies[dep] = fileSpec;
      changed = true;
    }
  }
...
async function spaceUpAction(options) {
    ...
    for (const [participantId, participant] of Object.entries(config.participants || {})) {
      ...
      if (participant.type === 'mcp-bridge' && participant.auto_start && participant.mcp_server) {
        ...
        try {
          // Find the bridge executable (prefer built dist, fallback to source)
          const distBridge = path.resolve(__dirname, '../../bin/mew-bridge.js');
          const legacyBridge = path.resolve(
            __dirname,
            '..',
            '..',
            '..',
          ...
          const bridgePath = fs.existsSync(distBridge) ? distBridge : legacyBridge;
