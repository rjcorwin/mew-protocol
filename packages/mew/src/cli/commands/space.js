...
function ensureLocalWorkspaceDependencies(spaceDir) {
  ...
  for (const [dep, targetPath] of Object.entries(targets)) {
    ...
    const linkSpec = normalized.startsWith('.') ? `file:${normalized}` : `file:./${normalized}`;
...
async function spaceUpAction(options) {
    ...
    for (const [participantId, participant] of Object.entries(config.participants || {})) {
      ...
      if (participant.type === 'mcp-bridge' && participant.auto_start && participant.mcp_server) {
        ...
        try {
          // Find the bridge executable (dist first, then source fallback)
          const bridgeCandidates = [
            path.resolve(__dirname, '..', '..', '..', 'bin', 'mew-bridge.js'),
            path.resolve(__dirname, '..', '..', '..', 'dist', 'bin', 'mew-bridge.js'),
            path.resolve(__dirname, '..', '..', '..', 'bridge', 'bin', 'mew-bridge.js'),
          ];

          const bridgePath = bridgeCandidates.find((candidate) => fs.existsSync(candidate));
          if (!bridgePath) {
            throw new Error('Unable to locate mew-bridge executable. Run `npm run build`.');
          }
