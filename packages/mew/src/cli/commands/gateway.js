  ...
  .action(async (options) => {
    ...
    function registerHttpParticipants() {
      ...
      for (const [participantId, config] of Object.entries(spaceConfig.participants)) {
        ...
        const configuredTokens = Array.isArray(config.tokens) ? config.tokens : [];
        const token = configuredTokens[0] ?? tokenMap.get(participantId);
        if (token) {
...
module.exports.createGatewayLogger = createGatewayLogger;
module.exports.parseOptionalBoolean = parseOptionalBoolean;
