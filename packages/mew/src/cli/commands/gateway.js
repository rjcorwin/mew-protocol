  ...
  .action(async (options) => {
    ...
    function registerHttpParticipants() {
      ...
      for (const [participantId, config] of Object.entries(spaceConfig.participants)) {
        const configuredTokens = Array.isArray(config.tokens)
          ? config.tokens.filter(Boolean)
          : [];
        let token = configuredTokens[0];

        if (!token && tokenMap.has(participantId)) {
          token = tokenMap.get(participantId);
        }
        ...
        if (!token) {
          ...
          tokenMap.set(participantId, token);
          console.warn(
            `Generated transient token for ${participantId}; consider running ` +
              '`mew space tokens` to persist it.',
          );
        }
        ...
        const isHttpParticipant =
          config.io === 'http' || (!config.io && !config.type && !config.command);
        if (isHttpParticipant) {
