...
function loadBridgeModule() {
  const distCandidate = path.resolve(__dirname, '../bridge/index.js');
  if (fs.existsSync(distCandidate)) {
    return require(distCandidate);
  }

  const buildCandidate = path.resolve(__dirname, '../../dist/bridge/index.js');
  if (fs.existsSync(buildCandidate)) {
    return require(buildCandidate);
  }

  throw new Error('Unable to locate MEW bridge implementation. Run "npm run build" or install @mew-protocol/mew.');
}

const { MCPBridge } = loadBridgeModule();
