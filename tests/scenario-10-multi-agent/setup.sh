#!/usr/bin/env bash
# Scenario 10 setup - prepares workspace for multi-agent coordination test
...
TEMPLATE_NAME=${TEMPLATE_NAME:-"scenario-10-multi-agent"}
...
node "${CLI_BIN}" init "${TEMPLATE_NAME}" --force --name "${SPACE_NAME}" --description "Scenario 10 - Multi-Agent" > init.log 2>&1
...
COORDINATOR_LOG=${WORKSPACE_DIR}/logs/coordinator.log
WORKER_LOG=${WORKSPACE_DIR}/logs/worker.log
ENV
