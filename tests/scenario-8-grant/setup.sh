#!/usr/bin/env bash
# Scenario 8 setup - prepares disposable workspace for capability grant workflow
...
TEMPLATE_NAME=${TEMPLATE_NAME:-"scenario-8-grant"}
...
node "${CLI_BIN}" init "${TEMPLATE_NAME}" --force --name "${SPACE_NAME}" --description "Scenario 8 - Capability Grant" > init.log 2>&1

mkdir -p logs workspace-files
...
AGENT_LOG=${WORKSPACE_DIR}/logs/test-agent.log
FILE_SERVER_LOG=${WORKSPACE_DIR}/logs/file-server.log
DATA_DIR=${WORKSPACE_DIR}/workspace-files
ENV
