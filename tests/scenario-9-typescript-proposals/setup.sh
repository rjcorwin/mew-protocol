...
echo "  - Try to send MCP request (should fail): echo '{\"kind\":\"mcp/request\",\"to\":[\"fulfiller-agent\"],\"payload\":{\"method\":\"tools/list\",\"params\":{}}}' > $FIFO_IN"
echo "  - Agent will send proposals instead"
echo "  - Fulfill proposals: echo '{\"kind\":\"mcp/request\",\"correlation_id\":[\"<proposal-id>\"],\"to\":[\"fulfiller-agent\"],\"payload\":{...}}' > $FIFO_IN"
