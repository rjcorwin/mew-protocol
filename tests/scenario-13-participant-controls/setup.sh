#!/usr/bin/env bash
# Scenario 13 setup - prepares workspace for participant control tests
...
TEMPLATE_NAME=${TEMPLATE_NAME:-"scenario-13-participant-controls"}
SPACE_NAME=${SPACE_NAME:-"scenario-13-participant-controls"}
...
node "${CLI_BIN}" init "${TEMPLATE_NAME}" --force --name "${SPACE_NAME}" --description "Scenario 13 - Participant Controls" > init.log 2>&1
