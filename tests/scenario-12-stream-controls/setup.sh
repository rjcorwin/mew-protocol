#!/usr/bin/env bash
# Scenario 12 setup - prepares workspace for stream control tests
...
TEMPLATE_NAME=${TEMPLATE_NAME:-"scenario-12-stream-controls"}
SPACE_NAME=${SPACE_NAME:-"scenario-12-stream-controls"}
...
node "${CLI_BIN}" init "${TEMPLATE_NAME}" --force --name "${SPACE_NAME}" --description "Scenario 12 - Stream Controls" > init.log 2>&1
