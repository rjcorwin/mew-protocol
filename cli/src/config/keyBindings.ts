...
export type KeyBinding = KeyPattern | KeyPattern[];
...
export function getBindingDisplay(binding: KeyBinding): string {
  if (Array.isArray(binding)) {
    return binding.map((entry) => getBindingDisplay(entry)).join(' or ');
  }
 ...
 */
export function getHelpText(bindings: KeyBindingMap = defaultKeyBindings): string[] {
 ...
 */
export function loadCustomBindings(customBindings: Partial<KeyBindingMap> = {}): KeyBindingMap {
  const sanitizedEntries = Object.entries(customBindings).filter(
    (entry): entry is [string, KeyBinding] => Boolean(entry[1]),
  );
  const sanitized = Object.fromEntries(sanitizedEntries) as KeyBindingMap;
 ...
 */
export function validateBindings(bindings: KeyBindingMap): BindingConflict[] {
  const warnings: BindingConflict[] = [];
  ...
  for (const [command, binding] of Object.entries(bindings)) {
    ...
    for (const pattern of patterns) {
      if (pattern.printable) {
        continue; // Skip printable char pattern
      }
